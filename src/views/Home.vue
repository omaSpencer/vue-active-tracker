	...
	<div v-if="dataLoaded" class="container mt-10 px-4">
		...
		<div v-else class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
			...
			>
				...
				/>
				<!-- Strength -->
				<img v-else src="@/assets/images/dumbbell-light-green.png" class="h-24 w-auto" alt="strength img" />
				<p class="mt-6 py-1 px-3 text-xs text-white bg-at-light-green shadow-md rounded-lg">
					{{ workout.workoutType }}
				</p>
				<h2 class="mt-8 mb-2 text-center text-xl text-at-light-green">{{ workout.workoutName }}</h2>
			</router-link>
		</div>
	</div>
</template>
